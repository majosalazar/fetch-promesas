window.onload = () => {
  const btnDoges = document.getElementById('doges');
  btnDoges.addEventListener('click', () => {
    obtenerDoges();
  })
  const btnCates = document.getElementById('cates');
  ...
  const btnAnimals = document.getElementById('dogCatBird');
  ...
  fetch(`https://cors-anywhere.herokuapp.com/http://shibe.online/api/birds?count=10&urls=true&httpsUrls=true`) //Recibe la URL donde se va a hacer la consulta
    .then((response) => { //Este then es de la promesa del fetch
      if (response.ok) {
        ...
        throw new Error("Mala respuesta de pajatitos");
      }
    ...
    .catch((error) => {
      console.error("Holi soy un error " + error);
      const errorBird = document.getElementById('birdReceptor');
      document.inner

    });
}

function dogesCatesBirds() {
  Promise.all([
